$(function() {
  ...
  if (deadlineId < parsedDeadlines.length) {
    ...
    if (confDeadline) {
      ...
      $('#ieee-satml2027-pract-ppml-sok-cnf-coreo-0 .timer').countdown(confDeadline.toDate(), make_update_countdown_fn(confDeadline));
      // check if date has passed, add 'past' class to it
      if (moment() - confDeadline > 0) {
        $('#ieee-satml2027-pract-ppml-sok-cnf-coreo-0').addClass('past');
      }
      $('#ieee-satml2027-pract-ppml-sok-cnf-coreo-0 .deadline-time').html(confDeadline.local().format('D MMM YYYY, h:mm:ss a'));
      deadlineByConf["ieee-satml2027-pract-ppml-sok-cnf-coreo-0"] = confDeadline;
    }
  } else {
    // TODO: hide the conf_id ?
  }
