$(function() {
  ...
  var rawDeadlines = ["2026-05-18 23:59"] || [];
  if (rawDeadlines.constructor !== Array) {
    rawDeadlines = [rawDeadlines];
  }
  var parsedDeadlines = [];
  ...
  if (deadlineId < parsedDeadlines.length) {
    ...
    if (confDeadline) {
      ...
      $('#ieee-cns2026-pract-cnf-coreo-0 .timer').countdown(confDeadline.toDate(), make_update_countdown_fn(confDeadline));
      // check if date has passed, add 'past' class to it
      if (moment() - confDeadline > 0) {
        $('#ieee-cns2026-pract-cnf-coreo-0').addClass('past');
      }
      $('#ieee-cns2026-pract-cnf-coreo-0 .deadline-time').html(confDeadline.local().format('D MMM YYYY, h:mm:ss a'));
      deadlineByConf["ieee-cns2026-pract-cnf-coreo-0"] = confDeadline;
    }
  } else {
    // TODO: hide the conf_id ?
  }
