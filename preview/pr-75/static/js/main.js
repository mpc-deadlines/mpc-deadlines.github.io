$(function() {
  ...
  if (deadlineId < parsedDeadlines.length) {
    ...
    if (confDeadline) {
      ...
      $('#watch2026-pract-sok-wk-0 .timer').countdown(confDeadline.toDate(), make_update_countdown_fn(confDeadline));
      // check if date has passed, add 'past' class to it
      if (moment() - confDeadline > 0) {
        $('#watch2026-pract-sok-wk-0').addClass('past');
      }
      $('#watch2026-pract-sok-wk-0 .deadline-time').html(confDeadline.local().format('D MMM YYYY, h:mm:ss a'));
      deadlineByConf["watch2026-pract-sok-wk-0"] = confDeadline;
    }
  } else {
    // TODO: hide the conf_id ?
  }
