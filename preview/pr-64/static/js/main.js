$(function() {
  ...
  // CONFERENCES.COMPUTER.ORG 2027
  
  var rawDeadlines = ["TBD"] || [];
  if (rawDeadlines.constructor !== Array) {
    rawDeadlines = [rawDeadlines];
  }
  var parsedDeadlines = [];
  ...
  if (deadlineId < parsedDeadlines.length) {
    ...
    if (confDeadline) {
      ...
      $('#conferences-computer-org2027-theory-pract-applied-cnf-exp-coreo-0 .timer').countdown(confDeadline.toDate(), make_update_countdown_fn(confDeadline));
      // check if date has passed, add 'past' class to it
      if (moment() - confDeadline > 0) {
        $('#conferences-computer-org2027-theory-pract-applied-cnf-exp-coreo-0').addClass('past');
      }
      $('#conferences-computer-org2027-theory-pract-applied-cnf-exp-coreo-0 .deadline-time').html(confDeadline.local().format('D MMM YYYY, h:mm:ss a'));
      deadlineByConf["conferences-computer-org2027-theory-pract-applied-cnf-exp-coreo-0"] = confDeadline;
    }
  } else {
    // TODO: hide the conf_id ?
  }
