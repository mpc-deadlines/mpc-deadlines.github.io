$(function() {
  ...
  if (deadlineId < parsedDeadlines.length) {
    ...
    if (confDeadline) {
      ...
      $('#trustfl-sis2026-pract-applied-wk-coreb-0 .timer').countdown(confDeadline.toDate(), make_update_countdown_fn(confDeadline));
      // check if date has passed, add 'past' class to it
      if (moment() - confDeadline > 0) {
        $('#trustfl-sis2026-pract-applied-wk-coreb-0').addClass('past');
      }
      $('#trustfl-sis2026-pract-applied-wk-coreb-0 .deadline-time').html(confDeadline.local().format('D MMM YYYY, h:mm:ss a'));
      deadlineByConf["trustfl-sis2026-pract-applied-wk-coreb-0"] = confDeadline;
    }
  } else {
    // TODO: hide the conf_id ?
  }
