$(function() {
  ...
  if (deadlineId < parsedDeadlines.length) {
    ...
    if (confDeadline) {
      ...
      $('#computer-conference2027-theory-pract-applied-cnf-exp-coreas-0 .timer').countdown(confDeadline.toDate(), make_update_countdown_fn(confDeadline));
      // check if date has passed, add 'past' class to it
      if (moment() - confDeadline > 0) {
        $('#computer-conference2027-theory-pract-applied-cnf-exp-coreas-0').addClass('past');
      }
      $('#computer-conference2027-theory-pract-applied-cnf-exp-coreas-0 .deadline-time').html(confDeadline.local().format('D MMM YYYY, h:mm:ss a'));
      deadlineByConf["computer-conference2027-theory-pract-applied-cnf-exp-coreas-0"] = confDeadline;
    }
  } else {
    // TODO: hide the conf_id ?
  }
