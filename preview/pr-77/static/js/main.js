$(function() {
  ...
  if (deadlineId < parsedDeadlines.length) {
    ...
    if (confDeadline) {
      ...
      $('#ct-rsa2027-theory-pract-applied-cnf-coreb-0 .timer').countdown(confDeadline.toDate(), make_update_countdown_fn(confDeadline));
      // check if date has passed, add 'past' class to it
      if (moment() - confDeadline > 0) {
        $('#ct-rsa2027-theory-pract-applied-cnf-coreb-0').addClass('past');
      }
      $('#ct-rsa2027-theory-pract-applied-cnf-coreb-0 .deadline-time').html(confDeadline.local().format('D MMM YYYY, h:mm:ss a'));
      deadlineByConf["ct-rsa2027-theory-pract-applied-cnf-coreb-0"] = confDeadline;
    }
  } else {
    // TODO: hide the conf_id ?
  }
