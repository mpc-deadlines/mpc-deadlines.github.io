$(function() {
  ...
  var rawDeadlines = ["2026-12-01 23:59"] || [];
  if (rawDeadlines.constructor !== Array) {
    rawDeadlines = [rawDeadlines];
  }
  var parsedDeadlines = [];
  ...
  if (deadlineId < parsedDeadlines.length) {
    ...
    if (confDeadline) {
      ...
      $('#ieee-security-privacy2027-applied-pract-jrn-corea-0 .timer').countdown(confDeadline.toDate(), make_update_countdown_fn(confDeadline));
      // check if date has passed, add 'past' class to it
      if (moment() - confDeadline > 0) {
        $('#ieee-security-privacy2027-applied-pract-jrn-corea-0').addClass('past');
      }
      $('#ieee-security-privacy2027-applied-pract-jrn-corea-0 .deadline-time').html(confDeadline.local().format('D MMM YYYY, h:mm:ss a'));
      deadlineByConf["ieee-security-privacy2027-applied-pract-jrn-corea-0"] = confDeadline;
    }
  } else {
    // TODO: hide the conf_id ?
  }
