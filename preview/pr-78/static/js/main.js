$(function() {
  ...
  var rawDeadlines = ["2026-07-01 23:59","2026-07-24 23:59"] || [];
  if (rawDeadlines.constructor !== Array) {
    rawDeadlines = [rawDeadlines];
  }
  var parsedDeadlines = [];
  while (rawDeadlines.length > 0) {
    ...
    var deadline = moment.tz(rawDeadline, "Etc/GMT-8");
  ...
  if (deadlineId < parsedDeadlines.length) {
    ...
    if (confDeadline) {
      ...
      $('#decaf2026-applied-ppml-wk-1 .timer').countdown(confDeadline.toDate(), make_update_countdown_fn(confDeadline));
      // check if date has passed, add 'past' class to it
      if (moment() - confDeadline > 0) {
        $('#decaf2026-applied-ppml-wk-1').addClass('past');
      }
      $('#decaf2026-applied-ppml-wk-1 .deadline-time').html(confDeadline.local().format('D MMM YYYY, h:mm:ss a'));
      deadlineByConf["decaf2026-applied-ppml-wk-1"] = confDeadline;
    }
  } else {
    // TODO: hide the conf_id ?
  }
